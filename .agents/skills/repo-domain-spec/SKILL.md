...
description: "Domain query trait conventions: one file per aggregate, Query vs RepoTransactional split, &self vs &mut self, reference params, RepoTransactional supertrait registration."
---
...
| `{Aggr}RepoTransactional` | `&mut self` | `TransactionalQuery<'c>` | Operations inside transactions |

`Query` 和 `TransactionalQuery` 的关系可以理解为：业务上需要事务保证的操作
放到 `*RepoTransactional`，不需要的留在 `*Query`。两者没有层级或继承关系，
各自保持最精简。
...
## 3. Method placement: Query vs RepoTransactional
...
- **`{Aggr}RepoTransactional`** — for operations that must run inside
...
Only `*RepoTransactional` traits use `&mut self`. The `*Query` trait
always uses `&self`.
...
`*Query` and `*RepoTransactional` are not CRUD interfaces — they are the
precise set of operations required by the use-case layer. When the use case
grows, add the new method then.

---

## 7. `RepoTransactional` supertrait — registration point

When a new aggregate gets a `*RepoTransactional` trait, two things must
happen in `src/domain/query.rs`:

**a) Add a `use` import:**
```rust
use crate::domain::query::new_aggregate::NewAggregateRepoTransactional;
```

**b) Add the trait as a supertrait bound on `RepoTransactional`:**
```rust
pub trait RepoTransactional:
    ...
    + NewAggregateRepoTransactional  // ← add here
{}
```
...
site because `RepoTransactional` does not imply the new trait.
...
```rust
query.run_in_transaction(async move |txn: &mut RepoTransactional<'_>| {
    Box::pin(async move {
...
The closure takes `&mut RepoTransactional<'_>`, through which all
`*RepoTransactional` methods are available via the supertrait chain.
