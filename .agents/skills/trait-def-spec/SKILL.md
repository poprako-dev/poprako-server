...
For repository traits, document whether the trait exposes independent `Execute` operations or transaction-bound `Advance` operations. Explain the role of `C` as the context anchor and link to `Nucl::coord` when it helps a reader understand the boundary.
