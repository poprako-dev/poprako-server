...
- `Query` and `RepoTransactional` structs
...
These are then called by both the `Query` and `RepoTransactional` impl blocks.
...
This keeps the `Query` / `RepoTransactional` impl blocks thin:
```rust
...
#[async_trait]
impl<'c> domain_query::user::UserRepoTransactional for RepoTransactional<'c> {
...
## Domain `RepoTransactional` trait aggregation

The domain's `RepoTransactional` trait in `domain/query.rs` aggregates all
per-entity `*Transactional` traits via supertraits:

```rust
pub trait RepoTransactional:
    Send + UserRepoTransactional + MemberRepoTransactional + MemberInvitationRepoTransactional
{
}
```

When adding a new entity with transactional operations, add its `*Transactional` trait
as a supertrait bound here so that `RepoTransactional` remains usable in
