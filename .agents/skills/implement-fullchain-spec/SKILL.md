...
### 9.1 Follow the API HTTP specification

Every API handler and router change must follow
`docs/how-to-implement-api-http.md` and the `api-http-spec` skill.

### 9.2 Use `Accept as _` for successful responses

Import the response helper as an anonymous trait and return success with
`.accept(...)`.
...
Do not import `Accept` directly, and do not use `Ok(HttpResponse::from(...))`
for new successful handler returns.

### 9.3 Use `?` for usecase errors

Usecase errors already convert into HTTP errors. Handlers must propagate them
directly:
...
Do not manually match ordinary usecase errors in HTTP handlers.

### 9.4 Handler names match usecase function names
...
### 10.1 Use standard RESTful route structure

Routes must use plural resource nouns and path params for resource identity.
Process-style routes are allowed only for genuinely procedural operations,
such as reserving an upload URL.

```rust
.route("/users/{user_id}", get(user::get_info))
.route("/users/me", get(user::get_my_info).put(user::update_info))
.route("/users/{user_id}/avatar/reserve", post(user::reserve_avatar))
```

### 10.2 Keep OpenAPI documentation synchronized

Every route must have a matching `#[utoipa::path(...)]` handler annotation.
Register documented handlers and schemas in `src/api/http/openapi.rs`.
Swagger UI and the OpenAPI JSON route must be exposed only in debug builds and
must live outside `/api/v1`, under `/api/swagger-ui` and `/api/openapi.json`.
