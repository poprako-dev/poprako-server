...
pub fn ensure_path_matches_body_id(path_id: &str, body_id: &str) -> Result<(), HttpError> {
    if path_id != body_id {
...
/// the path.
pub fn ensure_current_user(path_user_id: &str, token: &UserToken) -> Result<(), HttpError> {
