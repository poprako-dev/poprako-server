...
mod tests {
    ...
    // transaction_commits_query_and_prom(Drive::with_context)(positive): successful transactions should commit query and prom state together.
    // transaction_rolls_back_query_and_prom(Drive::with_context)(negative): failed transactions should discard query and prom state together.
    ...
    #[tokio::test]
    async fn transaction_commits_query_and_prom() {
        let mock = Mock::new();
        let form = MemberForm {
            id: "member-1".into(),
            user_id: "user-1".into(),
    ...
    #[tokio::test]
    async fn transaction_rolls_back_query_and_prom() {
        let mock = Mock::new();
        let form = MemberForm {
            id: "member-1".into(),
            user_id: "user-1".into(),
        ...
        let err = Drive::with_context(&mock, async move |context| {
            ...
            Err::<(), RootError>(unrecoverable(
                "[transaction_rolls_back_query_and_prom] fail",
            ))
        })
        .await
        .err()
