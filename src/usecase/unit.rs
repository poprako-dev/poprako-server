...
pub async fn list_infos<C, R>(
    ...
    data: unit_data::ListPageInfosData,
) -> RegularResult<unit_data::ListPageInfosVal>
where
    R: PageRepo<C>
        + UnitRepo<C>
        + ChapterRepo<C>
        ...
        + AssignmentRepoTransactional<C>,
{
    ...
    Ok(unit_data::ListPageInfosVal {
        unit_infos: unit_infos
            .into_iter()
            .map(unit_data::InfoVal::from)
            .collect(),
...
pub async fn save_infos<D, C, R>(
    ...
    repo: &R,
    token: user_model::Token,
    data: unit_data::SavePageData,
) -> RegularResult<unit_data::SavePageVal>
where
    D: Drive<C>,
    ...
    <R as DeriveTransactional>::Transactional: PageRepoTransactional<C>
        ...
        + Sync,
{
    let unit_data::SavePageData { page_id, diff } = data;
    ...
    let save_units = drive
        .with_context(
            async move |context| -> RegularResult<unit_data::SavePageVal> {
                //
                ...
                for oper in &opers {
                    match oper {
                        //
                        unit_model::Oper::Create { id, payload, .. } => {
                            repo.advance(
                                context,
                ...
                let old_counters = unit_model::Counters {
                    total_unit_count: page_info.total_unit_count,
                    translated_unit_count: page_info.translated_unit_count,
                    proofread_unit_count: page_info.proofread_unit_count,
                };
                ...
                Ok(unit_data::SavePageVal::from_parts(local_id_maps, counters))
            },
        )
        .await?;
...
struct UnitApplyParts {
    opers: Vec<unit_model::Oper>,
    local_id_maps: Vec<unit_model::IdMapper>,
}

impl From<unit_model::ApplyAck> for UnitApplyParts {
    fn from(receipt: unit_model::ApplyAck) -> Self {
        Self {
            opers: receipt.opers,
