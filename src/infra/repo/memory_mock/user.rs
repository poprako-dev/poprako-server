...
// };
// use crate::domain::repo_legacy::user::{UserRepo, UserRepoTransactional};
...
// 
// #[async_trait]
// impl UserRepo for MemoryMockQuery {
...
// }
// 
// // ── RepoTransactional impls ───────────────────────────────────────────────
// 
// #[async_trait]
// impl UserRepoTransactional for MemoryMockRepoTransactional {
...
// #[cfg(test)]
// mod tests {
//     // find_by_id_after_seed(UserRepo::get_by_id)(positive): seeded users should be found by ID.
//     // find_credential_by_qid_after_seed(UserRepo::get_credentials_by_qid)(positive): seeded credentials should be found by qualified ID.
//     // get_by_id_missing_returns_expected_error(UserRepo::get_by_id)(negative): missing users should return an expected argument error.
//     // create_then_find(UserRepoTransactional::create)(positive): created users should be readable after transaction commit.
//     // duplicate_qid_returns_conflict(UserRepoTransactional::create)(negative): duplicate qualified IDs should return an expected conflict.
//     // duplicate_nickname_returns_conflict(UserRepoTransactional::create)(negative): duplicate nicknames should return an expected conflict.
//     // update_changes_fields_and_is_readable_after_commit(UserRepoTransactional::update_info)(positive): updates should persist nickname and qid.
//     // update_missing_user_returns_expected_error(UserRepoTransactional::update_info)(negative): updating a non-existent user should fail.
//     // reserve_avatar_sets_key_and_version(UserRepoTransactional::reserve_avatar)(positive): reserve should set the avatar key and increment version.
//     // reserve_avatar_missing_user_returns_expected_error(UserRepoTransactional::reserve_avatar)(negative): reserving for a missing user should fail.
//     // mark_avatar_uploaded_sets_flag(UserRepoTransactional::mark_avatar_uploaded)(positive): marking should set avatar_uploaded to true.
//     // mark_avatar_uploaded_missing_user_returns_expected_error(UserRepoTransactional::mark_avatar_uploaded)(negative): marking for a missing user should fail.
//     // touch_last_active_updates_timestamp(UserRepo::touch_last_active)(positive): touching should update last_active_at.
//     // touch_last_active_missing_user_returns_expected_error(UserRepo::touch_last_active)(negative): touching for a missing user should fail.
//     // delete_removes_user_and_credentials(UserRepoTransactional::delete)(positive): deleting a user should remove the user and its credentials.
//     // delete_missing_returns_error(UserRepoTransactional::delete)(negative): deleting a missing user should fail.
// 
...
//         );
// 
//         let found = UserRepo::get_by_id(&mock, "user-1").await.unwrap();
...
//         );
// 
//         let cred = UserRepo::get_credentials_by_qid(&mock, "qid-1")
//             .await
//             .unwrap();
...
//         let mock = MemoryMockQuery::new();
// 
//         let err = UserRepo::get_by_id(&mock, "nonexistent")
//             .await
//             .err()
//             .unwrap();
...
//                     "pw".into(),
//                 );
//                 let user = UserRepoTransactional::create(txn, &form).await.unwrap();
...
//         let found = UserRepo::get_by_id(&mock, &mock.snapshot().users[0].id)
//             .await
//             .unwrap();
...
//                         "pw".into(),
//                     );
//                     UserRepoTransactional::create(txn, &form).await
//                 }
//                 .boxed()
//             })
//             .await
...
//                 };
//                 let updated = UserRepoTransactional::update_info(txn, &update)
//                     .await
//                     .unwrap();
...
//                         nickname: "n".into(),
//                     };
//                     UserRepoTransactional::update_info(txn, &update).await
//                 }
//                 .boxed()
//             })
//             .await
...
//             .transaction_scoped(|txn| {
//                 async move { UserRepoTransactional::reserve_avatar(txn, "user-1", "png").await }
//                     .boxed()
//             })
//             .await
//             .unwrap();
...
//             .transaction_scoped(|txn| {
//                 async move {
//                     UserRepoTransactional::reserve_avatar(txn, "nonexistent", "png").await
//                 }
//                 .boxed()
//             })
//             .await
...
//         mock.transaction_scoped(|txn| {
//             async move {
//                 UserRepoTransactional::reserve_avatar(txn, "user-1", "png").await?;
//                 UserRepoTransactional::mark_avatar_uploaded(txn, "user-1", 1).await
//             }
//             .boxed()
//         })
//         .await
...
//             .transaction_scoped(|txn| {
//                 async move {
//                     UserRepoTransactional::mark_avatar_uploaded(txn, "nonexistent", 1).await
//                 }
//                 .boxed()
//             })
//             .await
...
//         );
// 
//         let before = UserRepo::get_by_id(&mock, "user-1")
//             .await
//             .unwrap()
//             .last_active_at;
// 
...
// 
//         mock.transaction_scoped(|txn| {
//             async move { UserRepoTransactional::touch_last_active(txn, "user-1").await }.boxed()
//         })
//         .await
//         .unwrap();
// 
//         let after = UserRepo::get_by_id(&mock, "user-1")
//             .await
//             .unwrap()
//             .last_active_at;
//         assert!(after > before);
...
//         let err = mock
//             .transaction_scoped(|txn| {
//                 async move { UserRepoTransactional::touch_last_active(txn, "nonexistent").await }
//                     .boxed()
//             })
//             .await
//             .err()
...
// 
//         mock.transaction_scoped(|txn| {
//             async move { UserRepoTransactional::delete(txn, "user-1").await }.boxed()
//         })
//         .await
//         .unwrap();
// 
...
//         let err = mock
//             .transaction_scoped(|txn| {
//                 async move { UserRepoTransactional::delete(txn, "nonexistent").await }.boxed()
//             })
//             .await
//             .err()
//             .unwrap();
