...
// 
// #[async_trait]
// impl TeamRepo for MemoryMockQuery {
...
// 
// #[async_trait]
// impl TeamRepoTransactional for MemoryMockRepoTransactional {
...
// #[cfg(test)]
// mod tests {
//     // find_by_id_after_seed(TeamRepo::get_by_id)(positive): seeded teams should be found by ID.
//     // get_by_id_missing_returns_expected_error(TeamRepo::get_by_id)(negative): missing teams should return an expected argument error.
//     // list_returns_teams_ordered_by_created_at_desc(TeamRepo::list)(positive): list should return teams ordered by created_at DESC.
//     // reserve_avatar_sets_key_and_version(TeamRepoTransactional::reserve_avatar)(positive): reserve should set the avatar key, clear uploaded flag, and increment version.
//     // mark_avatar_uploaded_sets_flag(TeamRepo::mark_avatar_uploaded)(positive): marking should set avatar_uploaded to true.
//     // create_then_find(TeamRepo::create)(positive): created teams should be readable after transaction commit.
//     // create_duplicate_name_returns_conflict(TeamRepo::create)(negative): duplicate team names should return a conflict.
//     // update_changes_fields(TeamRepo::update)(positive): update should change name and description.
//     // update_missing_returns_error(TeamRepo::update)(negative): updating a missing team should fail.
//     // delete_removes_team(TeamRepo::delete)(positive): deleting a team should remove it from storage.
//     // delete_cascade_deletes_worksets(TeamRepo::delete)(positive): deleting a team should cascade-delete all worksets belonging to the team.
//     // delete_does_not_cascade_worksets_when_called_directly(TeamRepo::delete)(positive): calling delete directly before the cascade wrapper should NOT remove worksets — cascade is handled by complex::team::delete_cascade.
//     // delete_missing_returns_error(TeamRepo::delete)(negative): deleting a missing team should fail.
//     // increment_workset_next_index_returns_allocated(TeamRepoTransactional::increment_workset_next_index)(positive): each call should return the current value and increment it.
//     // increment_workset_next_index_missing_returns_error(TeamRepoTransactional::increment_workset_next_index)(negative): incrementing a missing team should fail.
// 
...
// 
//         let found = TeamRepo::get_by_id(&mock, "team-1").await.unwrap();
...
//         let mock = MemoryMockQuery::new();
// 
//         let err = TeamRepo::get_by_id(&mock, "nonexistent")
//             .await
//             .err()
//             .unwrap();
...
//                 async move { TeamRepoTransactional::reserve_avatar(txn, "team-1", "png").await }
//                     .boxed()
//             })
//             .await
//             .unwrap();
...
//         .unwrap();
// 
//         TeamRepo::mark_avatar_uploaded(&mock, "team-1", 1)
//             .await
//             .unwrap();
// 
//         let found = TeamRepo::get_by_id(&mock, "team-1").await.unwrap();
...
//         };
//         let team = TeamRepo::create(&mock, &form).await.unwrap();
...
//             description: "desc".into(),
//         };
//         TeamRepo::create(&mock, &form).await.unwrap();
// 
//         let form = TeamForm {
//             id: "team-2".into(),
//             name: "My Team".into(),
//             description: "desc".into(),
//         };
//         let err = TeamRepo::create(&mock, &form).await.err().unwrap();
// 
...
//         };
//         TeamRepo::update_info(&mock, &update).await.unwrap();
// 
//         let found = TeamRepo::get_by_id(&mock, "team-1").await.unwrap();
...
//             description: "Y".into(),
//         };
//         let err = TeamRepo::update_info(&mock, &update).await.err().unwrap();
// 
//         assert!(is_expected_argument(&err));
//     }
// 
...
//         mock.transaction_scoped(|txn| {
//             async move {
//                 TeamRepoTransactional::delete(txn, "team-1").await?;
//                 Ok(())
//             }
//             .boxed()
//         })
...
//         let err = mock
//             .transaction_scoped(|txn| {
//                 async move { TeamRepoTransactional::delete(txn, "nonexistent").await }.boxed()
//             })
//             .await
//             .err()
//             .unwrap();
...
//                 .transaction_scoped(|txn| {
//                     async move {
//                         TeamRepoTransactional::increment_workset_next_index(txn, "team-1").await
//                     }
//                     .boxed()
//                 })
//                 .await
...
//             .transaction_scoped(|txn| {
//                 async move {
//                     TeamRepoTransactional::increment_workset_next_index(txn, "nonexistent").await
//                 }
//                 .boxed()
//             })
//             .await
