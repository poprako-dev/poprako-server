...
// 
// #[async_trait]
// impl WorksetRepo for MemoryMockQuery {
...
// 
// #[async_trait]
// impl WorksetRepoTransactional for MemoryMockRepoTransactional {
...
// // ── Tests ──────────────────────────────────────────────────────────────────
// 
// // ── WorksetRepo forwarding on transactional handle ────────────────────────
// 
// #[async_trait]
// impl WorksetRepo for MemoryMockRepoTransactional {
//     async fn get_by_id(&self, id: &str) -> DomainResult<WorksetAggr> {
//         let state = self.state.lock().unwrap();
//         state
//             .worksets
...
// #[cfg(test)]
// mod tests {
//     // find_by_id_after_seed(WorksetRepo::get_by_id)(positive): seeded worksets should be found by ID.
//     // get_by_id_missing_returns_expected_error(WorksetRepo::get_by_id)(negative): missing worksets should return an expected argument error.
//     // list_filters_by_team_and_paginates(WorksetRepo::list)(positive): list should filter by team and paginate correctly.
//     // count_returns_correct_value(WorksetRepo::count)(positive): count should return the correct number of worksets for a team.
//     // create_then_find(WorksetRepoTransactional::create)(positive): created worksets should be readable after transaction commit.
//     // create_duplicate_index_returns_conflict(WorksetRepoTransactional::create)(negative): duplicate team+index should return an expected conflict.
//     // update_changes_fields(WorksetRepo::update)(positive): update should change name and description.
//     // update_missing_returns_error(WorksetRepo::update)(negative): updating a missing workset should fail.
//     // update_comic_count_applies_delta(WorksetRepoTransactional::update_comic_count)(positive): delta should be applied and clamped to zero.
//     // update_comic_count_missing_returns_error(WorksetRepoTransactional::update_comic_count)(negative): updating a missing workset should fail.
//     // increment_comic_next_index_returns_allocated(WorksetRepoTransactional::increment_comic_next_index)(positive): each call should return the current value and increment it.
//     // increment_comic_next_index_missing_returns_error(WorksetRepoTransactional::increment_comic_next_index)(negative): incrementing a missing workset should fail.
//     // delete_removes_workset(WorksetRepo::delete)(positive): deleting a workset should remove it from storage.
//     // delete_missing_returns_error(WorksetRepo::delete)(negative): deleting a missing workset should fail.
// 
...
// 
//         let found = WorksetRepo::get_by_id(&mock, "workset-1").await.unwrap();
...
//         let mock = MemoryMockQuery::new();
// 
//         let err = WorksetRepo::get_by_id(&mock, "nonexistent")
//             .await
//             .err()
//             .unwrap();
...
// 
//         let total = WorksetRepo::count(&mock, "team-1").await.unwrap();
//         assert_eq!(total, 2);
// 
//         let cnt_empty = WorksetRepo::count(&mock, "team-none").await.unwrap();
...
//                 };
//                 let created = WorksetRepoTransactional::create(txn, &form).await.unwrap();
...
//                     description: None,
//                 };
//                 WorksetRepoTransactional::create(txn, &form).await.unwrap();
//                 Ok(())
//             }
//             .boxed()
//         })
...
//                         description: None,
//                     };
//                     WorksetRepoTransactional::create(txn, &form).await
//                 }
//                 .boxed()
//             })
//             .await
...
//                 };
//                 WorksetRepo::update(txn, &update).await
//             }
//             .boxed()
//         })
//         .await
...
//         mock.transaction_scoped(|txn| {
//             async move {
//                 WorksetRepoTransactional::update_comic_count(txn, "workset-1", 3)
//                     .await
//                     .unwrap();
//                 Ok(())
//             }
...
//         mock.transaction_scoped(|txn| {
//             async move {
//                 WorksetRepoTransactional::update_comic_count(txn, "workset-1", -20)
//                     .await
//                     .unwrap();
//                 Ok(())
//             }
...
//             .transaction_scoped(|txn| {
//                 async move {
//                     WorksetRepoTransactional::update_comic_count(txn, "nonexistent", 1)
//                         .await
//                 }
//                 .boxed()
//             })
...
//                 .transaction_scoped(|txn| {
//                     async move {
//                         WorksetRepoTransactional::increment_comic_next_index(txn, "workset-1")
//                             .await
//                     }
//                     .boxed()
//                 })
...
//             .transaction_scoped(|txn| {
//                 async move {
//                     WorksetRepoTransactional::increment_comic_next_index(txn, "nonexistent").await
//                 }
//                 .boxed()
//             })
//             .await
...
// 
//         mock.transaction_scoped(|txn| {
//             async move { WorksetRepoTransactional::delete(txn, "workset-1").await }.boxed()
//         })
//         .await
//         .unwrap();
// 
...
//         let err = mock
//             .transaction_scoped(|txn| {
//                 async move { WorksetRepoTransactional::delete(txn, "nonexistent").await }.boxed()
//             })
//             .await
//             .err()
//             .unwrap();
