...
// 
// #[async_trait]
// impl MemberRepo for MemoryMockQuery {
...
// 
// #[async_trait]
// impl MemberRepoTransactional for MemoryMockRepoTransactional {
...
// #[cfg(test)]
// mod tests {
//     // duplicate_user_team_returns_conflict(MemberRepoTransactional::create)(negative): duplicate user-team membership should return an expected conflict.
...
//     // get_by_id_after_seed(MemberRepo::get_by_id)(positive): seeded members should be found by ID.
//     // get_by_id_missing_returns_expected_error(MemberRepo::get_by_id)(negative): missing members should return an expected argument error.
//     // get_by_user_and_team_id_after_seed(MemberRepo::get_by_user_and_team_id)(positive): seeded members should be found by user+team.
//     // get_by_user_and_team_id_missing_returns_expected_error(MemberRepo::get_by_user_and_team_id)(negative): missing members should return an expected argument error.
//     // list_filters_by_team_keyword_role(MemberRepo::list)(positive): list should filter by team, keyword, and role.
//     // exist_by_user_and_team_id_true(MemberRepo::exist_by_user_and_team_id)(positive): exist check should return true for existing member.
//     // exist_by_user_and_team_id_false(MemberRepo::exist_by_user_and_team_id)(positive): exist check should return false for non-existing member.
//     // list_user_filter_returns_user_members(MemberRepo::list)(positive): user filter should return all memberships for a user.
//     // update_roles_replaces_all_roles(MemberRepoTransactional::update_roles)(positive): update_roles should clear all roles and set only those in the mask.
//     // update_roles_missing_returns_error(MemberRepoTransactional::update_roles)(negative): updating roles on a missing member should fail.
//     // delete_removes_member(MemberRepoTransactional::delete)(positive): deleting a member should remove it from storage.
//     // delete_missing_returns_error(MemberRepoTransactional::delete)(negative): deleting a missing member should fail.
// 
...
//                 };
//                 MemberRepoTransactional::create(txn, &form).await.unwrap();
//                 Ok(())
//             }
//             .boxed()
//         })
...
//                     };
//                     MemberRepoTransactional::create(txn, &form).await
//                 }
//                 .boxed()
//             })
//             .await
...
//                     role_mask: RoleMask::from(RoleFlag::Admin),
//                 };
//                 MemberRepoTransactional::create(txn, &form1).await.unwrap();
// 
...
//                     role_mask: RoleMask::from(RoleFlag::Translator),
//                 };
//                 MemberRepoTransactional::create(txn, &form2).await.unwrap();
// 
//                 Ok(())
//             }
//             .boxed()
...
//         mock.transaction_scoped(|txn| {
//             async move {
//                 MemberRepoTransactional::update_user_nickname(
//                     txn,
//                     "user-1",
//                     "NewNick",
//                 )
...
//             .transaction_scoped(|txn| {
//                 async move {
//                     MemberRepoTransactional::update_user_nickname(txn, "no-such-user", "Nick")
//                         .await
//                 }
//                 .boxed()
//             })
...
//         ));
// 
//         let found = MemberRepo::get_by_id(&mock, "member-1").await.unwrap();
...
//         let mock = MemoryMockQuery::new();
// 
//         let err = MemberRepo::get_by_id(&mock, "nonexistent")
//             .await
//             .err()
//             .unwrap();
...
//         ));
// 
//         let found = MemberRepo::get_by_user_and_team_id(&mock, "user-1", "team-1")
//             .await
//             .unwrap();
//         assert_eq!(found.id, "member-1");
//     }
...
//         let mock = MemoryMockQuery::new();
// 
//         let err = MemberRepo::get_by_user_and_team_id(&mock, "user-1", "team-1")
//             .await
//             .err()
//             .unwrap();
//         assert!(is_expected_argument(&err));
...
// 
//         // List by team only.
//         let list = MemberRepo::list_by_team_id(
//             &mock,
//             "team-1",
//             None,
//             None,
...
// 
//         let exists = MemberRepo::exist_by_user_and_team_id(&mock, "u-1", "team-1")
//             .await
//             .unwrap();
//         assert!(exists);
//     }
...
//         ));
// 
//         let list = MemberRepo::list_by_user_id(
//             &mock,
//             "user-1",
//             Page {
//                 offset: 0,
...
//                     role_mask: new_roles,
//                 };
//                 MemberRepoTransactional::update_roles(txn, &update).await
//             }
//             .boxed()
//         })
//         .await
//         .unwrap();
// 
//         let found = MemberRepo::get_by_id(&mock, "m-1").await.unwrap();
//         assert!(!found.has_any_role(&[RoleFlag::Admin]));
...
// 
//         mock.transaction_scoped(|txn| {
//             async move { MemberRepoTransactional::delete(txn, "m-1").await }.boxed()
//         })
//         .await
//         .unwrap();
// 
...
//         let err = mock
//             .transaction_scoped(|txn| {
//                 async move { MemberRepoTransactional::delete(txn, "nonexistent").await }.boxed()
//             })
//             .await
//             .err()
//             .unwrap();
