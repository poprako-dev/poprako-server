...
// use crate::infra::repo::entity::member::{MemberAspect, MemberEntry, MemberRow};
// use crate::infra::repo::entity::team::TeamRow;
// use crate::infra::repo::entity::user::UserRow;
// use crate::infra::repo::schema::t_member::dsl::*;
// use crate::infra::repo::schema::t_team::dsl as team_dsl;
// use crate::infra::repo::schema::t_user::dsl as user_dsl;
...
//     let mut repo = t_member.filter(f_team_id.eq(team_id)).into_boxed();
// 
//     // Apply optional ILIKE filter on user_nickname.
//     if let Some(kw) = keyword {
//         let pattern = format!("%{}%", kw);
//         repo = repo.filter(f_user_nickname.ilike(pattern));
//     }
// 
//     // Apply optional role filter (IS NOT NULL on the corresponding column).
//     if let Some(flag) = role {
//         repo = match flag {
//             RoleFlag::RawProvider => repo.filter(f_assigned_raw_provider_at.is_not_null()),
//             RoleFlag::Translator => repo.filter(f_assigned_translator_at.is_not_null()),
//             RoleFlag::Proofreader => repo.filter(f_assigned_proofreader_at.is_not_null()),
//             RoleFlag::Typesetter => repo.filter(f_assigned_typesetter_at.is_not_null()),
//             RoleFlag::Redrawer => repo.filter(f_assigned_redrawer_at.is_not_null()),
//             RoleFlag::Reviewer => repo.filter(f_assigned_reviewer_at.is_not_null()),
//             RoleFlag::Publisher => repo.filter(f_assigned_publisher_at.is_not_null()),
//             RoleFlag::Admin => repo.filter(f_assigned_admin_at.is_not_null()),
//             RoleFlag::Assistant => repo.filter(f_assigned_assistant_at.is_not_null()),
//         };
//     }
// 
//     let rows: Vec<MemberRow> = repo
...
// 
// #[async_trait]
// impl<'c> MemberRepoTransactional for RdbRepoTransactional<'c> {
