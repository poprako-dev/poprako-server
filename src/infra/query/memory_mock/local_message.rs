...
#[async_trait]
impl LocalMessageQueryTransactional for MemoryMockQueryTransactional {
    ...
    async fn mark_transactional(&mut self, marks: &[&LocalMessageMark]) -> DomainResult<()> {
        ...
        LocalMessageQuery::mark(&query, marks).await
    }
}

#[cfg(test)]
mod tests {
    // append_then_claim_sets_processing_and_increments_lease(LocalMessageQueryTransactional::append/LocalMessageQuery::claim)(positive): claiming an appended message should set processing and increment lease.
    ...
    async fn append(mock: &MemoryMockQuery, form: LocalMessageForm) {
        Transactional::transaction_scoped(mock, |query| {
            async move {
                LocalMessageQueryTransactional::append(query, &form).await?;
                Ok(())
            }
            .boxed()
        })
