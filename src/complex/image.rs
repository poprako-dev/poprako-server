...
pub fn ensure_byte_length(
    ...
    let max_length = max_mib.saturating_mul(BYTES_PER_MIB);

    if !(1..=max_length).contains(&byte_length) {
        //
        return Err(invalid_byte_length_rejection(
            image_config,
            byte_length,
            kind,
        ));
...
fn parse_flat_key(
    ...
    (flat_key(prefix, id, ver, ext) == value).then_some((
        id.to_owned(),
        ext,
        ver,
    ))
