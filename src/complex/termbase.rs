...
use crate::complex::term as term_complex;
...
pub fn normalize_import(
    ...
    let mut seen_sources = HashSet::with_capacity(termbase_import.terms.len());

    let mut terms = Vec::with_capacity(termbase_import.terms.len());

    for term_import in termbase_import.terms {
        //
        let term_import = term_complex::build_import(
            term_import.source,
            term_import.targets,
            term_import.comment,
        )?;
...
pub fn find_import_target(
    ...
    termbase_infos
        .into_iter()
        .find(|termbase_info| normalized_key(&termbase_info.name) == name_key)
}
...
pub fn build_term_upsert_plan(
    ...
    for imported_term in imported_terms {
        ...
        let existing_term_info = existing_term_infos
            .iter()
            .find(|term_info| normalized_key(&term_info.source) == source_key);
    ...
    ensure_term_capacity(current_term_count, entries.len())?;
