...
| it_03 | `it_03_page_reserve_image.ts` | Authoritative hash-plus-extension manifests, optional `new_byte_len` retention, required upload lengths, duplicate-ID and count validation, object uploads, optimistic exact-generation mark semantics with immediate origin/thumbnail URLs, replacement, deletion, and page rebuilds. |
| it_04 | `it_04_assignment_invitation.ts` | Assignment joins, invitations, role updates, self role removal, and deletion. |
| it_05 | `it_05_unit_save_order_count.ts` | Unit v2 create/next ordering and counters; chapter-scoped non-empty single-character text search, stable ordering, and the 100-match limit; non-cascading literal transform; atomic combined translation exports; direct PopRaKo export/import replacement round trips. |
...
| it_08 | `it_08_info_update_upload_mark.ts` | Resource updates, runtime avatar-size validation, avatar/cover PUTs before optimistic exact-generation mark requests, immediate origin/thumbnail URLs, stale upload rejection, announcement create/update/delete, comments, and profiles. |
