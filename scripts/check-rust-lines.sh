...
for rust_file in $(find src poprako-util/src poprako-swagger/src -type f -name '*.rs' | LC_ALL=C sort); do
    ...
    case "$rust_file" in
        ...
        src/part_impl/repo/mock_impl/comic.rs) max_lines=707 ;;
        src/part_impl/repo/rdb_impl/comic/step_impl.rs) max_lines=665 ;;
        src/part_impl/repo/rdb_impl/team.rs) max_lines=617 ;;
        src/part_impl/repo/rdb_impl/user.rs) max_lines=647 ;;
